...
async function getResolvedVariantPermissions(fieldContext) {
  var _a;
  try {
    const result = await ((_a = import_visualBuilderPostMessage.default) == null ? void 0 : _a.send(import_postMessage.VisualBuilderPostMessageEvents.GET_RESOLVED_VARIANT_PERMISSIONS, fieldContext));
    return result ?? {
      update: true,
      error: true
    };
